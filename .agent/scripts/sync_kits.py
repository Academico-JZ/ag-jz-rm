...
def sync_kit(repo_url, kit_name):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Check Rules folder first, fallback to root
        ...
        local_gemini = os.path.join(AGENT_DIR, "rules", "GEMINI.md")
        ...
        if os.path.exists(remote_gemini) and os.path.exists(local_gemini):
            with open(remote_gemini, 'r', encoding='utf-8') as rf, open(local_gemini, 'r', encoding='utf-8') as lf:
                if rf.read() != lf.read():
                    ...
                    print(f"👉 Seu GEMINI.md customizado em {os.path.relpath(local_gemini, AGENT_DIR)} foi mantido.")
                    print(f"👉 Você pode comparar com a nova versão oficial em:")
